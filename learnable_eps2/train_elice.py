...
import sys
import importlib
# 1. HuggingFace datasets 먼저 강제 로드
hf_datasets = importlib.import_module("datasets")
# 2. sys.modules에 고정 (accelerate 보호)
...
_img_ds_path = os.path.join(DiT_path, "datasets", "img_latent_dataset.py")
ImgLatentDataset = SourceFileLoader(
    "img_latent_dataset", _img_ds_path
).load_module().ImgLatentDataset
import pickle
        ...
        for x, y in pbar:
            ...
            if cluster_type == 'kmeans':
                ...
                cluster_means, cluster_sigma, cluster_ids = get_cluster_gmm(
                    ...
                    gmm_covs,
                    gmm_weights,
                    stochastic=True  # stochastic=True면 posterior sampling, False면 argmax
                )
                cluster_sigma = torch.ones_like(x)
...
@torch.no_grad()
def get_cluster_gmm(
        x, y, gmm_means, gmm_covs, gmm_weights,
    ...
    for x, y in tqdm(
        ...
        if cluster_type == "kmeans":
            ...
            cluster_means, cluster_sigma, cluster_ids = get_cluster_gmm(
                ...
                gmm_covs,
                gmm_weights,
                stochastic=False  # stochastic=True면 posterior sampling, False면 argmax
            )
        else:
