...
file_path = "/home/ivpl-d26/ahlee/pycharm/winbuekbueq/research/generative_AI/leanable_latent_DiT/learnable_eps/output/lightningdit_b2_vavae_f16d3_lsun/lightningdit-b-2-ckpt-0013000-euler-1000/000000.safetensors" # inference output
...
def squeeze_or_fix(t: torch.Tensor):
    if t.shape[0] == 0:  # batch dimension이 0이면 강제로 [C, H, W]로 reshape
        return t.reshape(t.shape[1], t.shape[2], t.shape[3])
    elif t.shape[0] == 1:  # batch size 1일 경우 squeeze
        return t.squeeze(0)
    return t  # 이미 맞는 경우 그대로 반환
...
# Image.fromarray(z_img.cpu().numpy()).save(os.path.join(save_dir, "dit_z.png"))
# # Image.fromarray(z_org_img.cpu().numpy()).save(os.path.join(save_dir, "dit_z_org.png"))
# Image.fromarray(output_img.cpu().numpy()).save(os.path.join(save_dir, "dit_output.png"))
