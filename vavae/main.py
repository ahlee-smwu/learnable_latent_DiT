...
class SetupCallback(Callback):
    ...
    def on_pretrain_routine_start(self, trainer, pl_module):
        if trainer.global_rank == 0:
            ...
            while not os.path.exists(os.path.join('/output', self.logdir)):
                os.makedirs(os.path.join('/output', self.logdir), exist_ok=True)
                print(f"os.path.join('/output', {self.logdir}) does not exist, created it.")
            print(f"os.path.join('/output', {self.logdir}) exists")
            
            while not os.path.exists(os.path.join('/output', self.ckptdir)):
                os.makedirs(os.path.join('/output', self.ckptdir), exist_ok=True)
                print(f"os.path.join('/output', {self.ckptdir}) does not exist, created it.")
            print(f"os.path.join('/output', {self.ckptdir}) exists")
            
            while not os.path.exists(os.path.join('/output', self.cfgdir)):
                os.makedirs(os.path.join('/output', self.cfgdir), exist_ok=True)
                print(f"os.path.join('/output', {self.cfgdir}) does not exist, created it.")
            print(f"os.path.join('/output', {self.cfgdir}) exists")
